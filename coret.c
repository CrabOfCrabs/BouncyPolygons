...
ioctl(STDOUT_FILENO, TIOCGWINSZ, &w);
	...
	initscr();
        clear();
	Vec2 pt1 = {20,20},pt2 = {20,10},pt3 = {20,15};
	Tri tp = {&pt1,&pt2,&pt3};
	Vec2 force = {0.1,0.1};
	...
	while(1){
		...
		//gravacc =+ 0.1;
		Vec2 cen = tricenp(tp);rotateTri(&tp,cen,torque);

	//	force.y =force.y+ 0.01
			;move_Tri(&tp,force);

		torque = torque + border_Check(screen,tp.p1,&force,&tp,cen)/I;					
		torque = torque + border_Check(screen,tp.p2,&force,&tp,cen)/I;	
		torque = torque + border_Check(screen,tp.p3,&force,&tp,cen)/I;
